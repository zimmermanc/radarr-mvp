...
impl HDBitsApiAnalyzer {
    ...
    pub async fn analyze(&mut self) -> Result<()> {
        ...
        let exclusive_torrents = self
            .fetch_torrents_filtered(
                Some(vec![1]), // Movies
                None,          // All codecs
                None,          // All mediums
    ...
    async fn fetch_torrents_filtered(
        ...
        while page < self.config.max_pages {
            ...
            let mut params = vec![
                (
                    "limit".to_string(),
                    self.config.results_per_page.to_string(),
                ),
    ...
    fn analyze_torrents(&mut self, torrents: &[ApiTorrent]) -> Result<()> {
        info!(
            "Analyzing {} torrents for scene group patterns",
            torrents.len()
        );
        ...
        for torrent in torrents {
            ...
            let stats = self
                ...
                .or_insert_with(|| SceneGroupStats {
                    name: group_name,
                    release_count: 0,
                    total_size_gb: 0.0,
                    avg_seeders: 0.0,
            ...
            stats.avg_seeders = (stats.avg_seeders * (stats.release_count - 1) as f32
                + torrent.seeders as f32)
                / stats.release_count as f32;
            stats.avg_snatched = (stats.avg_snatched * (stats.release_count - 1) as f32
                + torrent.times_completed as f32)
                / stats.release_count as f32;
            ...
            if let Ok(re) = regex::Regex::new(pattern) {
                if let Some(captures) = re.captures(name) {
                    if let Some(group) = captures.get(1) {
                        ...
                        if ![
                            "1080p", "720p", "2160p", "x264", "x265", "h264", "h265", "BluRay",
                            "WEB", "HDTV",
                        ]
                        .contains(&group_name)
                        {
    ...
    async fn generate_reports(&self) -> Result<()> {
        ...
        for group in sorted_groups.iter().take(50) {
            // Top 50 groups
