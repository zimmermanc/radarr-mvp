...
impl HDBitsBrowseAnalyzer {
            ...
            while page < self.config.max_pages {
                ...
                if let Some(data) = api_response["data"].as_array() {
                    ...
                    for torrent_json in data {
                        if let Ok(torrent) =
                            serde_json::from_value::<HDBitsTorrent>(torrent_json.clone())
                        {
    ...
    fn calculate_group_metrics(metrics: &mut SceneGroupMetrics) {
        ...
        metrics.reputation_score = seeder_score
            + internal_score
            + completion_score
            + consistency_score
            + recency_score
    ...
    pub fn generate_analysis_report(&self, _start_time: DateTime<Utc>) -> BrowseAnalysisReport {
        ...
        report.internal_releases_analyzed =
            self.releases.iter().filter(|r| r.is_internal).count() as u32;
        ...
        report.top_internal_groups = top_groups
            .into_iter()
            .take(10)
            .map(|g| GroupSummary {
