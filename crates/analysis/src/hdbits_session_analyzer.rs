...
use crate::{HDBitsTorrent, ReleaseMetric, SceneGroupAnalyzer, SceneGroupMetrics};
...
impl HDBitsSessionAnalyzer {
    ...
    pub async fn collect_comprehensive_data(&self) -> Result<Vec<HDBitsTorrent>> {
        if !self.authenticated {
            return Err(anyhow::anyhow!(
                "Must be authenticated before collecting data"
            ));
        }
        ...
        for category in categories {
            ...
            for page in 0..self.config.max_pages {
                ...
                if all_torrents.len() >= 1000 {
                    info!(
                        "Collected sufficient data ({} torrents), stopping",
                        all_torrents.len()
                    );
                    break;
                }
        ...
        info!(
            "Data collection complete: {} total torrents",
            all_torrents.len()
        );
        Ok(all_torrents)
    }
    ...
    pub fn analyze_scene_groups(&mut self, releases: Vec<HDBitsTorrent>) -> Result<()> {
        ...
        for torrent in releases {
            ...
            if let Some(group_name) = SceneGroupAnalyzer::extract_scene_group(&torrent.name) {
                ...
                if !group_metrics
                    .categories_covered
                    .contains(&release_metric.category)
                {
                    group_metrics
                        .categories_covered
                        .push(release_metric.category.clone());
                }
            } else {
    ...
    pub fn generate_comprehensive_report(
        ...
        SessionAnalysisReport {
            ...
            internal_releases_analyzed: self.releases.iter().filter(|r| r.is_internal).count()
                as u32,
...
impl HDBitsSessionAnalyzer {
    ...
    fn calculate_reputation_score_static(metrics: &SceneGroupMetrics) -> f64 {
        ...
        let weighted_score = 0.25 * seeder_score
            + 0.20 * internal_score
