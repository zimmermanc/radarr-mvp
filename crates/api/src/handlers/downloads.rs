...
pub async fn list_downloads(
    ...
    let downloads = if let Some(movie_id) = params.movie_id {
        ...
        state
            .download_repo
            .find_by_movie_id(movie_id)
            .await
            .map_err(|e| ApiError::InternalError {
                message: format!("Failed to fetch downloads by movie: {}", e),
            })?
    } else {
        ...
        state
            ...
            .await
            .map_err(|e| ApiError::InternalError {
                message: format!("Failed to fetch downloads: {}", e),
            })?
    };

    let responses: Vec<DownloadResponse> =
        downloads.into_iter().map(DownloadResponse::from).collect();
...
pub async fn start_download(
    ...
    info!(
        "Starting download for GUID: {} from indexer: {}",
        request.guid, request.indexer_id
    );
    ...
    let saved_download =
        state
            ...
            .await
            .map_err(|e| ApiError::InternalError {
                message: format!("Failed to create download: {}", e),
            })?;

    info!(
        "Successfully created download with ID: {}",
        saved_download.id
    );
...
pub async fn get_download(
    ...
    let download = state
        ...
        .await
        .map_err(|e| ApiError::InternalError {
            message: format!("Failed to fetch download: {}", e),
        })?
        .ok_or_else(|| ApiError::NotFound {
            resource: format!("Download with ID: {}", download_id),
        })?;
