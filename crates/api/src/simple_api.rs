...
pub fn create_simple_api_router(state: SimpleApiState) -> Router {
    ...
    let api_router = Router::new()
        ...
        .route(
            "/v3/test/circuit-breaker/simulate-failure/:service",
            post(simulate_service_failure),
        )
        .route(
...
async fn search_movies(
    ...
    let search_result =
        perform_search_with_retry(indexer_client.as_ref(), &search_request, 3).await;
        ...
        Ok(is_healthy) => {
            if is_healthy {
                ...
                match indexer_client.get_indexers().await {
                    ...
                    Err(e) => {
                        warn!(
                            "Prowlarr health check passed but indexers fetch failed: {}",
                            e
                        );
                        let response = serde_json::json!({
                            "status": "warning",
...
async fn import_download(
    ...
    info!(
        "Import config: download_path={}, output_path={}, dry_run={}",
