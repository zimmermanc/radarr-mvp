...
impl FailureReason {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ...
            Self::ImportFailed(ImportFailureType::DirectoryCreationFailed) => {
                "Failed to create directory"
            }
            Self::ImportFailed(ImportFailureType::UnsupportedFormat) => "Unsupported file format",
            Self::ImportFailed(ImportFailureType::QualityAnalysisFailed) => {
                "Quality analysis failed"
            }
            Self::ImportFailed(ImportFailureType::FilenameParseFailed) => "Filename parse failed",
            Self::ImportFailed(ImportFailureType::MediaInfoFailed) => {
...
mod tests {
    ...
    fn test_failure_reason_retry_delays() {
        assert_eq!(
            FailureReason::AuthenticationFailed.default_retry_delay(),
            Duration::hours(6)
        );
        assert_eq!(
            FailureReason::DiskFull.default_retry_delay(),
            Duration::minutes(30)
        );
        assert_eq!(
            FailureReason::QualityRejected.default_retry_delay(),
            Duration::days(7)
        );
    }
