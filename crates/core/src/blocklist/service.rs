...
use crate::blocklist::repository::{BlocklistRepository, BlocklistStatistics, FailureReasonStat, IndexerHealthStatus};
...
impl<R: BlocklistRepository> BlocklistService<R> {
    ...
    pub async fn is_release_blocked(
        &self,
        release_id: &str,
        indexer: &str,
    ) -> Result<bool> {
    ...
    pub async fn search_blocked_releases(&self, query: &BlocklistQuery) -> Result<Vec<BlocklistEntry>> {
    ...
    pub async fn cleanup_old_entries(&self, older_than_days: i32) -> Result<i64> {
        let removed = self.repository.cleanup_expired_entries(older_than_days).await?;
    ...
    pub async fn check_indexer_health(
        ...
        let is_healthy = self.repository
    ...
    pub fn classify_error(&self, error: &RadarrError) -> FailureReason {
        match error {
            ...
            RadarrError::ExternalServiceError { service, error } => {
                ...
                } else if error_lower.contains("5") && (error_lower.contains("00") || error_lower.contains("02") || error_lower.contains("03")) {
    ...
    pub async fn get_expiring_entries(&self, within_hours: i32, limit: Option<i32>) -> Result<Vec<BlocklistEntry>> {
        self.repository.get_expiring_entries(within_hours, limit).await
    }
}
...
impl<R: BlocklistRepository> BlocklistIntegration for BlocklistService<R> {
    ...
    async fn get_indexer_failure_rate(&self, indexer: &str) -> Result<f64> {
        let health = self.check_indexer_health(indexer, Some(24), Some(10)).await?;
...
mod tests {
    ...
    fn test_error_classification() {
        ...
        assert_eq!(service.classify_error(&rate_limit_error), FailureReason::RateLimited);
    }
    ...
    impl BlocklistRepository for MockRepository {
        ...
        async fn get_entry(&self, _release_id: &str, _indexer: &str) -> Result<Option<BlocklistEntry>> {
            Ok(None)
        }
        ...
        async fn get_expiring_entries(&self, _within_hours: i32, _limit: Option<i32>) -> Result<Vec<BlocklistEntry>> {
            Ok(vec![])
        }
        ...
        async fn check_indexer_health(&self, _indexer: &str, _hours_back: i32, _failure_threshold: i32) -> Result<bool> {
            Ok(true)
        }
    }
}