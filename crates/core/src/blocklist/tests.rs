...
mod tests {
    ...
    impl BlocklistRepository for MockBlocklistRepository {
        ...
        async fn get_entry(
            ...
            release_id: &str,
            indexer: &str,
        ) -> crate::Result<Option<BlocklistEntry>> {
            let entries = self.entries.read().await;
            let key = (release_id.to_string(), indexer.to_string());
        ...
        ) -> crate::Result<Vec<BlocklistEntry>> {
            let entries = self.entries.read().await;
        ...
        async fn get_expired_entries(
            ...
            let entries = self.entries.read().await;
            let mut expired: Vec<BlocklistEntry> = entries
                .values()
        ...
        async fn get_expiring_entries(
            ...
            let mut expiring: Vec<BlocklistEntry> = entries
                .values()
        ...
        async fn get_failure_reason_stats(&self) -> crate::Result<Vec<FailureReasonStat>> {
            ...
            for entry in entries.values() {
                let (active, expired, retries) =
                    reason_counts.entry(entry.reason).or_insert((0, 0, vec![]));
        ...
        async fn get_recent_failure(
            ...
            let entries = self.entries.read().await;
            let mut matches: Vec<&BlocklistEntry> = entries
                .values()
    ...
    async fn test_is_release_blocked() {
        ...
        let blocked = service
            .is_release_blocked("test-release", "test-indexer")
            .await
            .unwrap();
        assert!(!blocked);
    ...
    async fn test_unblock_release() {
        ...
        let unblocked = service
            .unblock_release("test-release", "test-indexer", Some("Manual unblock"))
            .await
            .unwrap();
        assert!(unblocked);
    ...
    async fn test_retry_logic() {
        ...
        let entry = service
            .get_blocked_release("test-release", "test-indexer")
            .await
            .unwrap()
            .unwrap();
        assert_eq!(entry.retry_count, 0);
    ...
    async fn test_error_classification() {
        ...
        assert_eq!(
            service.classify_error(&rate_error),
        ...
        assert_eq!(
            service.classify_error(&external_error),
            FailureReason::ConnectionTimeout
        );
        ...
        assert_eq!(
            service.classify_error(&io_error),
    ...
    async fn test_indexer_health_check() {
        ...
        let health = service
            .check_indexer_health("test-indexer", Some(24), Some(5))
            .await
            .unwrap();
    ...
    async fn test_blocklist_integration_trait() {
        ...
        // Test report_failure
        let error = RadarrError::Timeout {
            operation: "test".to_string(),
        };
        service
            .report_failure("test-release", "test-indexer", &error, "Test Release", None)
            .await
            .unwrap();

        // Should be blocked now
        let should_skip = service
            .should_skip_release("test-release", "test-indexer")
            .await
            .unwrap();
        assert!(should_skip);

        // Test failure rate
        let failure_rate = service
            .get_indexer_failure_rate("test-indexer")
            .await
            .unwrap();
