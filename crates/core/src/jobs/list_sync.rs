...
impl ListSyncScheduler {
    ...
    pub async fn add_job(&self, job: SyncJob) -> Result<(), SyncError> {
        ...
        info!("Adding sync job for list {} ({})", job.list_name, job.list_id);
                        ...
                        if let Err(e) = scheduler.execute_job(job.clone()).await {
                            ...
                            if job.retry_count < job.max_retries {
                                ...
                                if let Some(job) = jobs.get_mut(&job.id) {
                                    ...
                                    warn!("Scheduled retry {} of {} for job {}", 
    ...
    async fn execute_job(&self, job: SyncJob) -> Result<(), SyncError> {
        ...
        info!("Executing sync job for list {} ({})", job.list_name, job.list_id);
        let start = Utc::now();
        ...
        match result {
            Ok(sync_result) => {
                info!("Sync job {} completed successfully: {} items found, {} added, {} updated", 
                      job.id, sync_result.items_found, sync_result.items_added, sync_result.items_updated);
                ...
                if let Some(job) = jobs.get_mut(&job.id) {
                    ...
                    info!("Next sync for {} scheduled at {}", job.list_name, job.next_sync);
                }
