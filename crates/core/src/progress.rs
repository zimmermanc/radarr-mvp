...
impl ProgressTracker {
    ...
    pub async fn start_operation(&self, operation_type: OperationType, title: impl Into<String>) -> Uuid {
    ...
    /// Update with step information
    pub async fn update_step(&self, id: Uuid, current: u32, total: u32, message: impl Into<String>) {
        let mut ops = self.operations.write().await;
        if let Some(progress) = ops.get_mut(&id) {
    ...
    pub async fn get_active_operations(&self) -> Vec<ProgressInfo> {
        let ops = self.operations.read().await;
        ops.values()
            .filter(|p| matches!(p.status, OperationStatus::InProgress | OperationStatus::Pending))
            .cloned()
            .collect()
    }
    ...
    pub async fn cleanup_old_operations(&self, older_than: Duration) {
        let mut ops = self.operations.write().await;
        let cutoff = Utc::now() - chrono::Duration::from_std(older_than).unwrap_or_else(|_| chrono::Duration::hours(1));
...
mod tests {
    ...
    async fn test_progress_tracker() {
        ...
        let id = tracker.start_operation(OperationType::Import, "Test Import").await;
