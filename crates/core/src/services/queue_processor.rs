    ...
    D: 'static,
{
    ...
    pub fn new(config: QueueProcessorConfig, queue_repo: Arc<Q>, download_client: Arc<D>) -> Self {
        let download_circuit_breaker = Arc::new(Mutex::new(CircuitBreaker::new(
            "download_client",
    ...
    async fn start_download(&self, queue_item: &crate::models::QueueItem) -> Result<()> {
        ...
        let progress_id = if let Some(tracker) = &self.progress_tracker {
            ...
            if let Some(bus) = &self.event_bus {
                let _ = bus
                    .publish(SystemEvent::ProgressUpdate {
                        operation_id: id,
        ...
        let client_id = circuit_breaker
            .execute(|| async {
                // Retry logic for adding download
                let retry_config = RetryConfig::slow();
                retry_with_backoff(retry_config, RetryPolicy::Transient, "add_download", || {
        ...
        if let Some(id) = progress_id {
            if let Some(tracker) = &self.progress_tracker {
                tracker
                    .update_progress(id, 10.0, "Download started, monitoring progress")
                    .await;
    ...
    async fn retry_failed_items(&self) -> Result<usize> {
        ...
        for mut item in retry_items {
            if item.can_retry() {
                info!(
                    "Retrying failed download: {} (attempt {}/{})",
                    item.title,
                    item.retry_count + 1,
                    item.max_retries
                );
...
mod tests {
    ...
    use crate::services::{ClientDownloadStatus, DownloadClientService, QueueRepository};
