...
impl<Q: QueueRepository, D: DownloadClientService> QueueService<Q, D> {
    ...
    pub async fn grab_release(
        ...
        if queue_item.priority == QueuePriority::High || queue_item.priority == QueuePriority::VeryHigh {
    ...
    pub async fn process_queue_item(&self, queue_id: Uuid) -> Result<()> {
        let mut queue_item = self.queue_repo.get_queue_item(queue_id).await?
    ...
    pub async fn remove_queue_item(&self, queue_id: Uuid, delete_files: bool) -> Result<()> {
        let queue_item = self.queue_repo.get_queue_item(queue_id).await?
            .ok_or_else(|| RadarrError::ValidationError {
                field: "queue_id".to_string(),
                message: format!("Queue item {} not found", queue_id),
            })?;
        ...
        if let Some(client_id) = &queue_item.download_client_id {
            let _ = self.download_client.remove_download(client_id, delete_files).await;
        }
    ...
    pub async fn get_queue_items(
        ...
        let mut items = if let Some(movie_id) = movie_id_filter {
            ...
            if let Some(status) = status_filter {
                all_items.into_iter().filter(|item| item.status == status).collect()
            } else {
                all_items
            }
        } else {
    ...
    pub async fn cleanup_completed_items(&self, days_old: i64) -> Result<usize> {
        ...
        let completed_items = self.queue_repo.get_queue_items(Some(QueueStatus::Completed)).await?;
...
mod tests {
    ...
    async fn test_grab_release() {
        ...
        let result = service.grab_release(
            &movie,
            &release,
    ...
    async fn test_process_queue_item() {
        ...
        let queue_item = service.grab_release(&movie, &release, None, None).await.unwrap();
        ...
        let updated_item = service.queue_repo.get_queue_item(queue_item.id).await.unwrap().unwrap();
