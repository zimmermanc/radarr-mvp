...
use radarr_core::{
    RadarrError, Result,
    models::{Movie, Download, DownloadStatus},
    domain::repositories::{MovieRepository, DownloadRepository},
...
use crate::{
    ImportPipeline, ImportConfig, ImportResult, ImportStats,
    FileScanner, AnalyzedFile, RenameEngine,
};
...
impl<M: MovieRepository, D: DownloadRepository> ImportService<M, D> {
    ...
    pub async fn process_download(&self, download_id: uuid::Uuid) -> Result<IntegratedImportResult> {
        info!("Processing download: {}", download_id);
        
        // Get download record
        let mut download = self.download_repo.find_by_id(download_id).await?
            .ok_or_else(|| RadarrError::NotFound { resource: format!("Download {} not found", download_id) })?;
        
        // Verify download is complete
        if download.status != DownloadStatus::Completed {
            return Err(RadarrError::ValidationError { 
                field: "status".to_string(),
                message: format!("Download {} is not completed (status: {:?})", download_id, download.status)
            });
        }
        ...
        info!("Importing from {} to {}", source_path.display(), dest_path.display());
        ...
        let (stats, import_results) = retry_with_backoff(
            ...
            "import_directory",
            || self.pipeline.import_directory_with_results(&source_path, &dest_path),
        ).await?;
        ...
        if import_success {
            if let Some(mut movie) = movie {
                self.update_movie_record(&mut movie, &dest_path, &import_results).await?;
            }
        }
    ...
    pub async fn process_all_pending(&self) -> Result<Vec<IntegratedImportResult>> {
        ...
        let pending_downloads = self.download_repo
    ...
    async fn update_movie_record(
        ...
        metadata.insert("path".to_string(), serde_json::json!(library_path.to_string_lossy()));
        
        // Find the main video file
        if let Some(main_file) = import_results.iter()
            .filter(|r| r.success)
            .find(|r| r.analyzed_file.is_some())
        {
            if let Some(analyzed) = &main_file.analyzed_file {
                ...
                metadata.insert("quality".to_string(), serde_json::json!({
    ...
    fn calculate_stats(&self, results: &[ImportResult]) -> ImportStats {
        ...
        let hardlinks_created = results.iter()
            .filter_map(|r| r.hardlink_result.as_ref())
            .filter(|h| h.is_hardlink)
            .count();
        let files_copied = results.iter()
            .filter_map(|r| r.hardlink_result.as_ref())
            .filter(|h| !h.is_hardlink)
            .count();
...
mod tests {
    ...
    async fn test_import_config_defaults() {
        ...
        assert_eq!(config.download_directory, PathBuf::from("/downloads/complete"));
