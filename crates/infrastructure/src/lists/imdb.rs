...
impl ImdbListParser {
    ...
    pub async fn parse_popular(&self) -> Result<Vec<ListItem>, ListParseError> {
        self.parse_list("https://www.imdb.com/chart/moviemeter")
            .await
    }
    ...
    fn extract_imdb_id(&self, url: &str) -> Option<String> {
        let patterns = [r"/title/(tt\d+)", r"(tt\d+)"];
    ...
    async fn parse_html_response(
        ...
        html: &str,
        source_url: &str,
    ) -> Result<Vec<ListItem>, ListParseError> {
        ...
        for (container_sel, title_sel, year_sel, rating_sel) in &selectors {
            ...
            if document.select(&container).count() > 0 {
                info!(
                    "Found {} items using selector: {}",
                    document.select(&container).count(),
                    container_sel
                );
                ...
                for element in document.select(&container) {
                    ...
                    if let Some(title_elem) = title_elem {
                        ...
                        let year = if !year_sel.is_empty() {
                            ...
                            element.select(&year_selector).next().and_then(|e| {
                        ...
                        let rating = if !rating_sel.is_empty() {
                            ...
                            element.select(&rating_selector).next().and_then(|e| {
                                e.text().collect::<String>().trim().parse::<f32>().ok()
                            })
                        } else {
                            None
                        };
        ...
        if items.is_empty() {
            ...
            return Err(ListParseError::ParseError(
                "No items found in list".to_string(),
            ));
        }
    ...
    async fn fetch_and_parse(&self, url: &str) -> Result<Vec<ListItem>, ListParseError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(ListParseError::HttpError(reqwest::Error::from(
...
impl ListParser for ImdbListParser {
    async fn parse_list(&self, list_url: &str) -> Result<Vec<ListItem>, ListParseError> {
        if !self.validate_url(list_url) {
            return Err(ListParseError::InvalidUrl(format!(
                "Invalid IMDb URL: {}",
                list_url
            )));
        }
