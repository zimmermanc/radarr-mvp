...
impl CachedTmdbClient {
    ...
    pub async fn search_movies(&self, query: &str, page: Option<i32>) -> Result<Vec<Movie>, crate::tmdb::TmdbError> {
        debug!("Searching TMDB movies: query={}, page={:?}", query, page);
        self.client.search_movies(query, page).await
    }
    
    pub async fn get_popular(&self, page: Option<i32>) -> Result<Vec<Movie>, crate::tmdb::TmdbError> {
        debug!("Fetching TMDB popular movies: page={:?}", page);
        self.client.get_popular(page).await
    }
    
    pub async fn get_upcoming(&self, page: Option<i32>) -> Result<Vec<Movie>, crate::tmdb::TmdbError> {
