...
async fn main() -> Result<()> {
    ...
    let api_key = env::var("RADARR_API_KEY").unwrap_or_else(|_| "mysecurekey123".to_string());
    ...
    let selected_release = search_results
        .results
        .iter()
    ...
    loop {
        ...
        print!(
            "📥 {} | Progress: {:.1}% | ↓ {} | ↑ {}    ",
            current_item.status, progress, download_speed, upload_speed
        );
