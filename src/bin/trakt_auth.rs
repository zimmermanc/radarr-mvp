...
struct Args {
    ...
    #[arg(long, env = "DATABASE_URL", default_value = "postgresql://radarr:radarr@localhost:5432/radarr")]
    database_url: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Poll for token
    match oauth_client.poll_for_token_with_retry(
        ...
        Ok(token_response) => {
            ...
            println!("Access Token: {}...", &token_response.access_token[..20.min(token_response.access_token.len())]);
