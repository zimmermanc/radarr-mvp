...
async fn test_circuit_breaker_basic_functionality() {
    ...
    for i in 0..3 {
        let result = circuit_breaker.call(async {
            Ok::<String, RadarrError>(format!("Success {}", i + 1))
        }).await;
    ...
    for i in 0..4 {
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!("Request {}: Circuit breaker opened (service: {})", i + 4, service);
                break;
            }
            other => {
    ...
    let rejected_result = circuit_breaker.call(async {
        Ok::<String, RadarrError>("Should not execute".to_string())
    }).await;
    ...
    println!("Final metrics: {} total, {} successful, {} failed, {} rejected", 
...
async fn test_circuit_breaker_recovery() {
    ...
    let recovery_result = circuit_breaker.call(async {
        Ok::<String, RadarrError>("Recovery successful".to_string())
    }).await;
    ...
    let post_recovery_result = circuit_breaker.call(async {
        Ok::<String, RadarrError>("Post recovery request".to_string())
    }).await;
    ...
    println!("Recovery metrics: {} total, {} successful, {} failed", 
             final_metrics.total_requests, final_metrics.successful_requests, 
...
async fn test_circuit_breaker_timeout_functionality() {
    ...
    let timeout_result = circuit_breaker.call(async {
    ...
    assert!(elapsed < Duration::from_millis(100), "Should timeout quickly: {:?}", elapsed);
...
async fn test_circuit_breaker_manual_control() {
    ...
    let rejected_result = circuit_breaker.call(async {
        Ok::<String, RadarrError>("Should be rejected".to_string())
    }).await;
    ...
    let success_result = circuit_breaker.call(async {
        Ok::<String, RadarrError>("Now should work".to_string())
    }).await;
...
async fn test_circuit_breaker_health_check() {
    ...
    let config = CircuitBreakerConfig::new("health_service")
        .with_failure_threshold(3);
    let circuit_breaker = CircuitBreaker::new(config);
    ...
    let final_metrics = circuit_breaker.get_metrics().await;
    println!("Health check metrics: success_rate={:.1}%, circuit_state={:?}", 
...
async fn test_different_error_types() {
    ...
    let config = CircuitBreakerConfig::new("error_types_service")
        .with_failure_threshold(2);
    let circuit_breaker = CircuitBreaker::new(config);
    
    let error_types = vec![
        RadarrError::NetworkError { message: "Network failure".to_string() },
        RadarrError::ExternalServiceError { 
            service: "test_service".to_string(), 
            error: "Service error".to_string() 
        },
        RadarrError::TemporaryError { message: "Temporary issue".to_string() },
    ...
    for (i, error) in error_types.iter().enumerate() {
        let result = circuit_breaker.call(async {
            Err::<String, RadarrError>(error.clone())
        }).await;
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!("Error type {}: Circuit breaker opened (service: {})", i + 1, service);
                break;
            }
            other => {
    ...
    let metrics = circuit_breaker.get_metrics().await;
    println!("Error types metrics: {} total, {} failed", 
...
async fn test_concurrent_circuit_breaker_access() {
    ...
    let config = CircuitBreakerConfig::new("concurrent_service")
    ...
    println!("Concurrent results: {} successes, {} failures, {} rejections", 
             successes, failures, rejections);
    
    let metrics = circuit_breaker.get_metrics().await;
    assert_eq!(metrics.total_requests, 10);
    assert_eq!(metrics.successful_requests as usize + metrics.failed_requests as usize + metrics.rejected_requests as usize, 10);
    
    println!("Final concurrent metrics: {} total, {} successful, {} failed, {} rejected", 
             metrics.total_requests, metrics.successful_requests, 
             metrics.failed_requests, metrics.rejected_requests);
...
async fn test_fault_injection_complete_workflow() {
    ...
    let indexer_result = indexer_cb.call(async {
        Ok::<String, RadarrError>("Indexer search results".to_string())
    }).await;
    
    let downloader_result = downloader_cb.call(async {
        Ok::<String, RadarrError>("Download started".to_string())
    }).await;
    ...
    for i in 0..4 {
        let result = indexer_cb.call(async {
            Err::<String, RadarrError>(RadarrError::ExternalServiceError {
    ...
    for i in 0..3 {
        let result = downloader_cb.call(async {
    ...
    assert!(indexer_state == CircuitBreakerState::Open || downloader_state == CircuitBreakerState::Open);
    ...
    let indexer_recovery = indexer_cb.call(async {
        Ok::<String, RadarrError>("Indexer recovered".to_string())
    }).await;
    
    let downloader_recovery = downloader_cb.call(async {
        Ok::<String, RadarrError>("Downloader recovered".to_string())
    }).await;
    ...
    println!("\n--- Final Metrics ---");
    println!("Indexer: {} total, {} success, {} failed, {} rejected", 
             indexer_metrics.total_requests, indexer_metrics.successful_requests,
             indexer_metrics.failed_requests, indexer_metrics.rejected_requests);
    println!("Downloader: {} total, {} success, {} failed, {} rejected",
             downloader_metrics.total_requests, downloader_metrics.successful_requests,
             downloader_metrics.failed_requests, downloader_metrics.rejected_requests);
