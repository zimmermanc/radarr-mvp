...
impl WorkflowCoordinator {
        ...
        let search_response = SearchResponse {
            ...
            results: vec![
                ...
                ProwlarrSearchResult {
                    ...
                    download_url: "magnet:?xt=urn:btih:d23fe1c06bba254a9dc9f519b335aa7c1367a99b".to_string(),
    ...
    pub async fn execute_movie_acquisition_workflow(&self, movie: &Movie) -> radarr_core::Result<String> {
        println!("🎬 Starting movie acquisition workflow for: {}", movie.title);
        ...
        println!("✅ Found {} releases from {} indexers", 
        ...
        for progress in [0.1, 0.3, 0.6, 0.8, 1.0] {
            self.download_client.update_progress(&torrent_hash, progress);
            let status = self.download_client.get_torrent_status(&torrent_hash).await?;
    ...
    pub async fn test_error_scenarios(&self) -> radarr_core::Result<()> {
        ...
        self.download_client.should_fail.store(true, std::sync::atomic::Ordering::Relaxed);
        
        let result = self.download_client.add_torrent(AddTorrentParams::default()).await;
        assert!(result.is_err());
        println!("✅ Download client gracefully handled failure");
        
        // Reset failure state
        self.download_client.should_fail.store(false, std::sync::atomic::Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_complete_movie_acquisition_workflow() {
        ...
        let result = coordinator.execute_movie_acquisition_workflow(&test_movie).await;
        ...
        assert_eq!(torrents[0].state, "completed", "Should be completed after simulation");
    }
    ...
    async fn test_release_selection_algorithm() {
        ...
        let search_response = coordinator.indexer_client.search(&search_request).await.unwrap();
        
        let best_release = coordinator.select_best_release(&search_response.results).unwrap();
        
        // Should prefer 1080p BluRay release due to good quality and reasonable size
        assert!(best_release.title.contains("1080p"), "Should prefer 1080p release");
        assert!(best_release.title.contains("BluRay"), "Should prefer BluRay source");
        
        // Should have good metrics
        assert!(best_release.seeders.unwrap_or(0) > 10, "Should have good seeders");
        assert!(best_release.size.unwrap_or(0) > 0, "Should have size information");
    }
    ...
    async fn test_error_handling_scenarios() {
        ...
        assert!(result.is_ok(), "Error scenarios should be handled gracefully");
    }
    ...
    async fn test_download_progress_monitoring() {
        ...
        let hash = coordinator.download_client.add_torrent(torrent_params).await.unwrap();
        
        // Test progress updates
        for progress in [0.25, 0.5, 0.75, 1.0] {
            coordinator.download_client.update_progress(&hash, progress);
            let status = coordinator.download_client.get_torrent_status(&hash).await.unwrap();
            
            if let Some(torrent) = status {
                assert!((torrent.progress - progress).abs() < 0.01, "Progress should match");
                if progress >= 1.0 {
