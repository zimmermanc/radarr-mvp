        ...
        for i in 0..threshold + 5 {
            ...
            match result {
                Err(RadarrError::CircuitBreakerOpen { .. }) => {
                    println!("  Circuit opened after {} failures (threshold: {})", failures, threshold);
                    assert!(failures >= threshold, "Circuit should open after {} failures, opened after {}", threshold, failures);
                    break;
                }
                Err(_) => {
                    failures += 1;
...
async fn test_circuit_breaker_timeout_behavior() {
    ...
    for timeout_duration in timeout_durations {
        ...
        let remaining_time = timeout_duration.saturating_sub(open_time.elapsed()) + Duration::from_millis(20);
        ...
        assert!(context.wait_for_circuit_state(CircuitBreakerState::HalfOpen, Duration::from_millis(100)).await,
                "Circuit should transition to half-open after timeout: {:?}", timeout_duration);
...
async fn test_circuit_breaker_success_threshold() {
    ...
    for success_threshold in success_thresholds {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_json(json!({"status": "recovered"}))
            )
        ...
        assert_eq!(context.circuit_breaker.get_state().await, CircuitBreakerState::Closed,
                   "Circuit should be closed after {} successful requests", success_threshold);
        
        println!("  ✓ Success threshold {} working correctly", success_threshold);
    }
}
...
async fn test_circuit_breaker_concurrent_behavior() {
    ...
    context.setup_always_failing_endpoint("/api/concurrent").await;
    ...
    assert!(circuit_open_count > 0, "Should have some circuit breaker rejections");
    assert!(failure_count + circuit_open_count == 10, "All requests should fail somehow");
    ...
    println!("Concurrent circuit breaker: {} failures, {} rejections", failure_count, circuit_open_count);
}
...
async fn test_circuit_breaker_metrics_accuracy() {
    ...
    for (i, (url, should_succeed)) in request_pattern.iter().enumerate() {
        ...
        match (result, should_succeed) {
            ...
            (result, expected) => {
                println!("Request {}: Unexpected result {:?} (expected success: {})", 
                         i + 1, result.is_ok(), expected);
            }
        }
    ...
    let expected_rejection_rate = (expected_rejections as f64 / request_pattern.len() as f64) * 100.0;
    ...
    println!("  Total: {} (expected: {})", metrics.total_requests, request_pattern.len());
    println!("  Success: {} (expected: {})", metrics.successful_requests, expected_successes);
    println!("  Failures: {} (expected: {})", metrics.failed_requests, expected_failures);
    println!("  Rejections: {} (expected: {})", metrics.rejected_requests, expected_rejections);
    println!("  Rates - Success: {:.1}%, Failure: {:.1}%, Rejection: {:.1}%", 
...
async fn test_circuit_breaker_health_check() {
    ...
    context.setup_always_failing_endpoint("/api/unhealthy").await;
    ...
    assert!(context.circuit_breaker.is_healthy().await, "Should be healthy with no requests");
    ...
    assert!(context.circuit_breaker.is_healthy().await, "Should be healthy with all successes");
    
    // Make some failures (but not enough to open circuit)
    let _ = context.make_request(&unhealthy_url).await;
    assert!(context.circuit_breaker.is_healthy().await, "Should still be healthy with one failure");
    ...
    assert!(!context.circuit_breaker.is_healthy().await, "Should be unhealthy with circuit open");
    
    let metrics = context.get_test_metrics().await;
    println!("Health test metrics: {} successful, {} failed, circuit state: {:?}", 
...
async fn test_circuit_breaker_stress_test() {
    ...
    context.setup_intermittent_failure_endpoint("/api/stress", 10).await;
    ...
    println!("  Success: {}, Failures: {}, Rejections: {}", success_count, failure_count, rejection_count);
    ...
    assert_eq!(cb_metrics.successful_requests as usize + cb_metrics.failed_requests as usize + cb_metrics.rejected_requests as usize, request_count);
    
    // System should be resilient (not completely broken)
    let metrics = context.get_test_metrics().await;
    assert!(metrics.is_resilient(), "System should demonstrate resilience under stress");
}
...
mod circuit_breaker_integration_tests {
    ...
    async fn test_multiple_service_circuit_breakers() {
        ...
        service_b.setup_intermittent_failure_endpoint("/api/data", 3).await;
        ...
        assert_eq!(state_a, CircuitBreakerState::Open, "Service A should have open circuit");
        // Service B state depends on intermittent failures
        assert_eq!(state_c, CircuitBreakerState::Closed, "Service C should have closed circuit");
        ...
        println!("  Service A: state={:?}, failures={}", state_a, metrics_a.failed_requests);
        println!("  Service B: state={:?}, successes={}, failures={}", state_b, metrics_b.successful_requests, metrics_b.failed_requests);
        println!("  Service C: state={:?}, successes={}", state_c, metrics_c.successful_requests);
    }
}