    ...
    if let Err(error) = result {
        match error {
            ...
            other => {
                ...
                println!("Got error (may be valid depending on implementation): {:?}", other);
            }
        }
    }
    ...
    let result = context.circuit_breaker.call(async {
        ...
        if content_type.contains("application/json") {
            if let Err(_) = serde_json::from_str::<serde_json::Value>(&body) {
                return Err(RadarrError::SerializationError(
                    format!("Response claims to be JSON but contains invalid data: {}", body)
                ));
            }
        }
...
async fn test_corrupted_download_file() {
    ...
    let client = reqwest::Client::new();
    let verify_result = context.circuit_breaker.call(async {
        let response = client.post(&verify_url)
            .json(&json!({
                "file_path": "/downloads/Test Movie 2024.mkv",
                "expected_hash": "abc123def456"
            }))
        ...
        if !verification["is_valid"].as_bool().unwrap_or(true) {
            return Err(RadarrError::IoError(
                format!("File corruption detected: {}", 
                    verification["error"].as_str().unwrap_or("Unknown corruption"))
            ));
        }
    ...
    if let Err(RadarrError::IoError(msg)) = verify_result {
        ...
        panic!("Expected IoError for file corruption, got: {:?}", verify_result);
    }
    ...
    for i in 0..6 {
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!("Circuit breaker opened after {} corruptions", corruption_count);
...
async fn test_corruption_recovery() {
    ...
    for i in 0..3 {
        ...
        assert!(result.is_err(), "Request {} should fail due to corruption", i + 1);
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
    ...
    let recovery_result = context.circuit_breaker.call(async {
        let response = client.get(&status_url)
            .send()
            .await
            .map_err(|e| RadarrError::NetworkError { 
        ...
        let status: serde_json::Value = serde_json::from_str(&body)
            .map_err(|e| RadarrError::SerializationError(format!("JSON parse error: {}", e)))?;
...
async fn test_intermittent_corruption() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"status": "ok", "request": 1}))
        )
        .up_to_n_times(1)
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"status": "ok", "request": 3}))
        )
        .up_to_n_times(1)
        .mount(&context.mock_server)
        .await;
    ...
    assert!(failed_count > 0, "Should have some failed requests due to corruption");
    
    println!("Intermittent corruption results:");
    for (request_num, success) in results {
        println!("  Request {}: {}", request_num, if success { "SUCCESS" } else { "FAILED" });
    }
    
    println!("Final metrics - successful: {}, failed: {}, total: {}", 
...
async fn test_various_corruption_types() {
    ...
    for (endpoint, corruption_type) in test_cases {
        ...
        let result = context.circuit_breaker.call(async {
            ...
            serde_json::from_str::<serde_json::Value>(&body)
                .map_err(|e| RadarrError::SerializationError(
                    format!("{}: {}", corruption_type, e)
                ))?;
    ...
    assert!(corruption_types_detected >= 3, "Should detect multiple corruption types");
...
mod corruption_integration_tests {
    ...
    async fn test_corruption_in_search_workflow() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_string("{ \"results\": [ {\"title\": corrupted_data_without_quotes } ] }")
                    .insert_header("content-type", "application/json")
            )
        ...
        let client = reqwest::Client::new();
        let search_result = search_service.circuit_breaker.call(async {
            ...
            let _results: serde_json::Value = serde_json::from_str(&body)
                .map_err(|e| RadarrError::SerializationError(format!("Search JSON error: {}", e)))?;
            
            Ok::<String, RadarrError>(body)
        }).await;
        
        assert!(search_result.is_err(), "Search should fail due to corruption");
        ...
        println!("Workflow handled partial corruption correctly - search failed, metadata succeeded");
    }
}