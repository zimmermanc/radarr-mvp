...
async fn test_disk_space_monitoring() {
    ...
    if let Ok(body) = result {
        ...
        assert!(percentage_used > 90, "Should indicate high disk usage: {}%", percentage_used);
        
        println!("Disk space check: {} bytes available, {}% used", available, percentage_used);
    }
...
async fn test_download_failure_disk_full() {
    ...
    let add_result = context.circuit_breaker.call(async {
        ...
        if status.is_server_error() {
            ...
            if body.contains("No space left") || body.contains("ENOSPC") {
                return Err(RadarrError::IoError("Disk full: No space left on device".to_string()));
            }
...
async fn test_import_failure_disk_full() {
    ...
    // Then attempt import
    let client = reqwest::Client::new();
    let import_result = context.circuit_breaker.call(async {
    ...
    if let Err(RadarrError::IoError(msg)) = import_result {
        ...
        panic!("Expected IoError for insufficient disk space, got: {:?}", import_result);
    }
...
async fn test_download_pause_on_low_disk_space() {
    ...
    let client = reqwest::Client::new();
    let pause_result = client.post(&pause_url)
        .form(&[("hashes", "pause1")])
        .send()
        .await;
...
async fn test_disk_space_recovery() {
    ...
    for i in 0..2 {
        ...
        if let Ok(body) = result {
            ...
            println!("Phase 1 check {}: {}% disk usage (critical)", i + 1, percentage_used);
        }
    ...
    if let Ok(body) = recovery_result {
        ...
        println!("Space recovered: {}GB available, {}% used", available / 1024 / 1024 / 1024, percentage_used);
    }
    
    // Resume downloads
    let client = reqwest::Client::new();
    let resume_result = client.post(&resume_url)
        .form(&[("hashes", "all")])
        .send()
        .await;
...
async fn test_concurrent_operations_disk_full() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(507)
                .set_body_json(json!({"error": "No space left on device"}))
        )
        .mount(&context.mock_server)
        .await;
    ...
    handles.push(tokio::spawn(async move {
        let client = reqwest::Client::new();
        let result = context_ref.circuit_breaker.call(async {
    ...
    for result in results {
        ...
        match response {
            ...
            Err(RadarrError::IoError(msg)) if msg.contains("Disk full") => {
                disk_full_errors += 1;
                println!("Operation {} failed with disk full error: {}", operation, msg);
            }
            Err(other) => {
                println!("Operation {} failed with other error: {:?}", operation, other);
            }
        }
    }
    ...
    assert!(disk_full_errors >= 2, "Import and add torrent should fail with disk full");
    ...
    println!("Concurrent disk full test - successful: {}, disk full errors: {}", successful_ops, disk_full_errors);
}
...
async fn test_disk_full_alert_generation() {
    ...
    for i in 0..3 {
        ...
        if let Ok(body) = result {
            ...
            println!("Disk space check {}: {} bytes available (critical)", i + 1, available);
        }
...
mod disk_integration_tests {
    ...
    async fn test_complete_disk_space_workflow() {
        ...
        for (phase_name, expected_gb, expected_percentage) in phases {
            let result = context.make_request(&space_url).await;
            assert!(result.is_ok(), "Space check should succeed in {} phase", phase_name);
            ...
            if let Ok(body) = result {
                ...
                assert!(available_gb == expected_gb || available_gb < 1, 
                    "Phase {}: expected ~{}GB, got {}GB", phase_name, expected_gb, available_gb);
                assert_eq!(percentage_used, expected_percentage, 
                    "Phase {}: expected {}% used, got {}%", phase_name, expected_percentage, percentage_used);
                
                println!("Phase {}: {}GB available, {}% used", phase_name, available_gb, percentage_used);
            }
