...
async fn test_concurrent_download_stalls() {
    ...
    for result in results {
        ...
        if let Ok(body) = response {
            ...
            println!("Request {}: {} stalled, {} downloading", request_id, stalled_matches, downloading_matches);
        }
    }
...
async fn test_download_bandwidth_throttling() {
    ...
    if avg_speed <= 2048 { // Less than 2KB/s average is considered throttling
        throttling_detected = true;
    }
    
    assert!(throttling_detected, "Should have detected bandwidth throttling");
    println!("Average download speed: {} B/s (throttling detected: {})", avg_speed, throttling_detected);
...
async fn test_download_cleanup_after_failure() {
    ...
    let client = reqwest::Client::new();
    let cleanup_result = client.post(&delete_url)
        .form(&[
            ("hashes", "cleanup1"),
            ("deleteFiles", "false")
        ])
...
mod download_integration_tests {
    ...
    async fn test_complete_download_failure_workflow() {
        ...
        // Step 1: Add torrent
        let client = reqwest::Client::new();
        let add_result = download_client.circuit_breaker.call(async {
            let response = client.post(&add_url)
        ...
        for i in 0..2 {
            ...
            assert!(result.is_ok(), "Initial monitoring {} should succeed", i + 1);
            tokio::time::sleep(Duration::from_millis(50)).await;
        }
        ...
        assert_eq!(download_client.circuit_breaker.get_state().await, CircuitBreakerState::Open);
