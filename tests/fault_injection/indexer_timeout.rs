...
async fn test_partial_indexer_timeout_scenario() {
    ...
    let fast_requests = results
        .iter()
        .filter(|(_, _, elapsed)| *elapsed < Duration::from_millis(500))
        .count();
    let slow_requests = results
        .iter()
        .filter(|(_, _, elapsed)| *elapsed >= Duration::from_millis(500))
        .count();
...
async fn test_cascading_indexer_timeouts() {
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        if let Err(error) = result {
            match error {
                ...
                other => panic!(
                    "Expected timeout error for indexer {}, got: {:?}",
                    i + 1,
                    other
                ),
            }
    ...
    assert_eq!(
        indexer1.circuit_breaker.get_state().await,
        CircuitBreakerState::Closed
    );
    assert_eq!(
        indexer2.circuit_breaker.get_state().await,
        CircuitBreakerState::Closed
    );
    assert_eq!(
        indexer3.circuit_breaker.get_state().await,
        CircuitBreakerState::Closed
    );
}
...
async fn test_indexer_timeout_alert_generation() {
    ...
    println!(
        "Alerts generated for critical indexer timeout: {}",
        has_alerts
    );
...
async fn test_indexer_timeout_with_retry_backoff() {
    ...
    context
        .setup_intermittent_failure_endpoint("/api/v1/search", 2)
        .await;
    ...
    assert!(metrics.successful_requests > 0);
    assert!(
        attempts.iter().any(|(_, success, _)| *success),
