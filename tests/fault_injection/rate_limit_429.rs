...
async fn test_rate_limit_without_retry_after() {
    ...
    Mock::given(method("GET"))
        .and(path("/api/data"))
        .respond_with(
            ResponseTemplate::new(429).set_body_string("Rate limit exceeded - no retry info"),
        )
    ...
    for i in 0..6 {
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!(
                    "Circuit breaker opened after {} rate limits",
                    rate_limit_count
                );
...
async fn test_concurrent_requests_rate_limited() {
    ...
    for result in results {
        ...
        match response {
            Err(RadarrError::RateLimited { .. }) => {
                rate_limit_count += 1;
                println!(
                    "Concurrent request {} rate limited in {:?}",
                    request_id, elapsed
                );
            }
            Err(RadarrError::CircuitBreakerOpen { .. }) => {
                ...
                println!(
                    "Concurrent request {} rejected by circuit breaker in {:?}",
                    request_id, elapsed
                );
            }
            other => {
                println!(
                    "Concurrent request {} result: {:?} in {:?}",
    ...
    println!(
        "Rate limited requests: {}, Circuit breaker rejections: {}",
        rate_limit_count, circuit_open_count
    );
...
async fn test_burst_rate_limiting() {
    ...
    println!(
        "Burst results: {} successful, {} failed",
...
async fn test_rate_limit_error_message_handling() {
    ...
    }) = result
    {
        ...
        println!(
            "Parsed rate limit error - service: {}, retry_after: {:?}",
            service, retry_after
        );
    } else {
...
mod rate_limit_integration_tests {
    ...
    async fn test_multiple_services_different_rate_limits() {
        ...
        fast_service
            .setup_rate_limited_endpoint("/api/fast", 1)
            .await;

        // Slow service: high rate limit (retry after 10 seconds)
        slow_service
            .setup_rate_limited_endpoint("/api/slow", 10)
            .await;
        ...
        if let Err(RadarrError::RateLimited {
            retry_after: Some(fast_retry),
            ..
        }) = fast_result
        {
            if let Err(RadarrError::RateLimited {
                retry_after: Some(slow_retry),
                ..
            }) = slow_result
            {
                ...
                assert!(
                    slow_retry > fast_retry,
                    "Slow service should have longer retry period"
                );

                println!(
                    "Fast service retry: {}s, Slow service retry: {}s",
