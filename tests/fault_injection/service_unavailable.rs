...
async fn test_http_503_service_unavailable() {
    ...
    if let Err(error) = result {
        match error {
            RadarrError::ExternalServiceError { service, error: err_msg } => {
...
async fn test_connection_refused_service_down() {
    ...
    let result = context.circuit_breaker.call(async {
        let response = client.get(&nonexistent_url)
    ...
    for i in 0..6 {
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!("Circuit breaker opened after {} unavailable responses", unavailable_count);
...
async fn test_service_recovery_after_outage() {
    ...
    for i in 0..3 {
        ...
        assert!(result.is_err(), "Request {} should fail - service unavailable", i + 1);
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
...
async fn test_intermittent_service_availability() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"pong": true, "sequence": 1}))
        )
        .up_to_n_times(1)
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"pong": true, "sequence": 3}))
        )
        .up_to_n_times(1)
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(503)
                .set_body_string("Service overloaded")
        )
        .up_to_n_times(1)
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"pong": true, "sequence": 5}))
        )
        .mount(&context.mock_server)
        .await;
    ...
    for (request_num, available) in results {
        println!("  Request {}: {}", request_num, if available { "AVAILABLE" } else { "UNAVAILABLE" });
    }
    
    println!("Final metrics - available: {}, unavailable: {}", available_count, unavailable_count);
}
...
async fn test_concurrent_requests_service_down() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(503)
                .set_body_string("All systems down for maintenance")
        )
        .mount(&context.mock_server)
        .await;
    ...
    for result in results {
        ...
        match response {
            Err(RadarrError::ExternalServiceError { .. }) => {
                unavailable_count += 1;
                println!("Concurrent request {} service unavailable in {:?}", request_id, elapsed);
            }
            Err(RadarrError::TemporaryError { .. }) => {
                unavailable_count += 1;
                println!("Concurrent request {} temporary error in {:?}", request_id, elapsed);
            }
            Err(RadarrError::CircuitBreakerOpen { .. }) => {
                circuit_open_count += 1;
                println!("Concurrent request {} circuit breaker open in {:?}", request_id, elapsed);
            }
            other => {
                println!("Concurrent request {} unexpected result: {:?} in {:?}", request_id, other, elapsed);
            }
        }
    }
    ...
    assert!(unavailable_count + circuit_open_count == 5, 
        "All requests should fail: {} unavailable, {} circuit open", unavailable_count, circuit_open_count);
    
    // Requests should fail relatively quickly
    assert!(avg_time < Duration::from_secs(2), "Average response time should be fast: {:?}", avg_time);
    
    let metrics = context.get_test_metrics().await;
    assert_eq!(metrics.total_requests, 5);
    assert!(metrics.failed_requests > 0 || metrics.rejected_requests > 0);
    
    println!("Concurrent outage results - unavailable: {}, circuit open: {}, avg time: {:?}", 
...
async fn test_service_degradation_levels() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(503)
                .set_body_string("Minor service degradation")
        )
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(502)
                .set_body_string("Bad Gateway - upstream service failed")
        )
        .mount(&context.mock_server)
        .await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(504)
                .set_body_string("Gateway timeout - service unreachable")
        )
        .mount(&context.mock_server)
        .await;
    ...
    for (endpoint, expected_error) in degradation_levels {
        ...
        if let Err(error) = result {
            match error {
                ...
                RadarrError::TemporaryError { message } => {
                    println!("Detected as temporary error for {}: {}", expected_error, message);
                }
                other => {
...
async fn test_health_check_during_outage() {
    ...
    let client = reqwest::Client::new();
    let health_result = context.circuit_breaker.call(async {
...
mod service_outage_integration_tests {
    ...
    async fn test_cascading_service_outage() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(503)
                    .set_body_string("Indexer maintenance in progress")
            )
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(502)
                    .set_body_string("Upstream metadata provider failed")
            )
        ...
        assert!(metadata_result.is_err(), "Metadata service should be unavailable");
        
        // Download service should still work
        assert!(download_result.is_ok(), "Download service should still be available");
    ...
    async fn test_complete_service_restoration() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(503)
                    .set_body_string("All services offline")
            )
            .up_to_n_times(4)
            .mount(&context.mock_server)
            .await;
        ...
        for i in 0..4 {
            let result = context.make_request(&status_url).await;
            assert!(result.is_err(), "Request {} should fail during outage", i + 1);
            tokio::time::sleep(Duration::from_millis(10)).await;
        }
