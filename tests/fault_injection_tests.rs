...
mod integration_tests {
    ...
    async fn test_complete_fault_injection_workflow() {
        ...
        assert!(download_result.is_ok(), "Download client should work normally");
        assert!(metadata_result.is_ok(), "Metadata service should work normally");
        ...
        download_client.setup_rate_limited_endpoint("/status", 30).await;
        ...
        assert!(download_failures > 0, "Download client should experience rate limits");
        assert!(metadata_failures > 0, "Metadata service should be unavailable");
        
        println!("✓ Fault injection working - {} indexer, {} download, {} metadata failures", 
        ...
        println!("Circuit breaker states: Indexer={:?}, Download={:?}, Metadata={:?}",
        ...
        assert!(open_circuits > 0, "At least one circuit breaker should have opened");
        ...
        println!("✓ {} out of 3 services recovered successfully", recovered_services);
        ...
        println!("Download: {} total, {} success, {} failed, {} rejected",
                 download_metrics.total_requests, download_metrics.successful_requests,
                 download_metrics.failed_requests, download_metrics.rejected_requests);
        println!("Metadata: {} total, {} success, {} failed, {} rejected",
                 metadata_metrics.total_requests, metadata_metrics.successful_requests,
                 metadata_metrics.failed_requests, metadata_metrics.rejected_requests);
    ...
    async fn test_fault_injection_modules_loaded() {
        ...
        println!("✓ All {} fault injection modules loaded successfully", contexts.len());
        
        // Verify each context is functional
        for (i, context) in contexts.iter().enumerate() {
            let metrics = context.get_test_metrics().await;
            assert_eq!(metrics.total_requests, 0, "Context {} should start with 0 requests", i);
            assert_eq!(metrics.successful_requests, 0, "Context {} should start with 0 successes", i);
            assert_eq!(metrics.failed_requests, 0, "Context {} should start with 0 failures", i);
        }
