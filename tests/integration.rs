...
use common::{create_test_app, create_test_movie, TestContext};
...
async fn test_complete_movie_workflow() {
    ...
    let response = server.post("/api/v3/movie").json(&movie_request).await;
    ...
    assert!(
        best_release.download_url.starts_with("magnet:")
            || best_release.download_url.starts_with("http")
    );
    ...
    assert!(
        download_completed,
        "Download should complete within test timeout"
    );
    ...
    let updated_movie_response = server.get(&format!("/api/v3/movie/{}", movie_id)).await;

    assert_eq!(updated_movie_response.status_code(), StatusCode::OK);

    let updated_movie: MovieResponse = updated_movie_response.json();
    assert!(
        updated_movie.has_file,
        "Movie should have file after import"
    );
    assert!(
        updated_movie.movie_file_id.is_some(),
        "Movie should have file ID"
    );
...
async fn test_invalid_movie_id_error_handling() {
    ...
    let response = server.get(&format!("/api/v3/movie/{}", invalid_id)).await;
...
async fn test_external_service_failures() {
    ...
    let search_response = server
        .get(&format!(
            "/api/v3/release?movieId={}&fail_prowlarr=true",
            movie.id
        ))
        .await;
...
async fn test_database_error_handling() {
    ...
    let response1 = server.post("/api/v3/movie").json(&movie_request).await;
    assert_eq!(response1.status_code(), StatusCode::CREATED);

    // Second creation should fail with conflict
    let response2 = server.post("/api/v3/movie").json(&movie_request).await;
...
async fn test_api_validation() {
    ...
    let response = server.post("/api/v3/movie").json(&invalid_request).await;
    ...
    let response = server.post("/api/v3/movie").json(&zero_request).await;
    ...
    let response = server.post("/api/v3/movie").json(&incomplete_request).await;

    assert_eq!(response.status_code(), StatusCode::BAD_REQUEST);

    // Test invalid UUID in path
    let response = server.get("/api/v3/movie/invalid-uuid").await;
...
async fn test_concurrent_operations() {
    ...
    let movie_requests: Vec<_> = (1..=10)
        .map(|i| CreateMovieRequest {
            tmdb_id: 1000 + i,
    ...
    let list_response = server.get("/api/v3/movie?page=1&pageSize=20").await;
...
async fn test_performance_under_load() {
    ...
    let total_duration: std::time::Duration = results.iter().map(|r| r.as_ref().unwrap().1).sum();
...
async fn test_pagination() {
    ...
    for i in 1..=25 {
        ...
        let response = server.post("/api/v3/movie").json(&request).await;
        assert_eq!(response.status_code(), StatusCode::CREATED);
    }

    // Test first page
    let page1_response = server.get("/api/v3/movie?page=1&pageSize=10").await;
    ...
    // Test second page
    let page2_response = server.get("/api/v3/movie?page=2&pageSize=10").await;
    ...
    let invalid_page_response = server.get("/api/v3/movie?page=0&pageSize=10").await;
    assert_eq!(invalid_page_response.status_code(), StatusCode::BAD_REQUEST);

    let large_page_response = server.get("/api/v3/movie?page=1000&pageSize=10").await;
...
async fn test_search_functionality() {
    ...
    let quality_search_response = server
        .get(&format!(
            "/api/v3/release?movieId={}&quality=1080p",
            movie.id
        ))
        .await;
    ...
    let category_search_response = server
        .get(&format!(
            "/api/v3/release?movieId={}&categories=2000,2010",
            movie.id
        ))
        .await;
    ...
    let invalid_search_response = server
        .get(&format!(
            "/api/v3/release?movieId={}&quality=invalid",
            movie.id
        ))
        .await;

    assert_eq!(
        invalid_search_response.status_code(),
