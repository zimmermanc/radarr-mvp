...
async fn test_prometheus_metrics_endpoint() {
    ...
    assert!(
        body_str.contains("radarr_list_sync"),
        "Should contain radarr metrics"
    );
    assert!(
        body_str.contains("sync_operations_total"),
        "Should contain sync operation metrics"
    );
    assert!(
        body_str.contains("api_requests_total"),
        "Should contain API request metrics"
    );
    assert!(
        body_str.contains("cache_hits_total"),
        "Should contain cache metrics"
    );

    // Verify generation timestamp
    assert!(
        body_str.contains("# Generated at"),
...
async fn test_monitoring_status_endpoint() {
    ...
    let body_bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
        ...
        .unwrap();
    let body_str = String::from_utf8(body_bytes.to_vec()).unwrap();
    let status_response: MonitoringStatusResponse =
        serde_json::from_str(&body_str).expect("Should parse monitoring status response");
...
async fn test_metrics_endpoint_performance() {
    ...
    assert!(
        ...
        "Should handle {} concurrent requests in under 5 seconds",
        concurrent_requests
    );
    ...
    for line in lines {
        if line.starts_with("# HELP") {
            ...
            assert_eq!(
                ...
                "TYPE should match previous HELP metric"
            );
...
async fn test_status_response_json_structure() {
    ...
    let json_value: Value = serde_json::from_str(&body_str).expect("Response should be valid JSON");
    ...
    assert!(
        ...
        "Should contain startedAt field"
    );
    assert!(
        obj.contains_key("uptimeSeconds"),
        "Should contain uptimeSeconds field"
    );
    assert!(
        obj.contains_key("totalOperationsMonitored"),
        "Should contain totalOperationsMonitored field"
    );
    assert!(
        obj.contains_key("syncMetrics"),
        "Should contain syncMetrics field"
    );
    assert!(
        obj.contains_key("alertStats"),
        "Should contain alertStats field"
    );
    assert!(
        obj.contains_key("healthSummary"),
        "Should contain healthSummary field"
    );
    assert!(
        obj.contains_key("circuitBreakerStatus"),
        "Should contain circuitBreakerStatus field"
    );
    ...
    let circuit_breaker_status = obj
        .get("circuitBreakerStatus")
        .unwrap()
        .as_object()
        .unwrap();
    assert!(
        !circuit_breaker_status.is_empty(),
        "Should have circuit breaker data"
    );
...
async fn test_endpoint_response_times() {
    ...
    let endpoints = vec!["/metrics", "/api/v3/monitoring/status"];
