...
impl IndexerClient for MockHDBitsClient {
    async fn search(&self, _request: &SearchRequest) -> RadarrResult<SearchResponse> {
        ...
        *self.request_count.lock().unwrap() += 1;
        ...
        let response = responses.get("default").cloned().unwrap_or_else(|| SearchResponse {
            total: 0,
            results: vec![],
            indexers_searched: 1,
            indexers_with_errors: 0,
...
impl SearchService {
    ...
    pub async fn search_and_rank(&self, request: &SearchRequest) -> RadarrResult<Vec<ReleaseScore>> {
        let search_response = self.hdbits_client.search(request).await?;
        
        let scored_releases = search_response.results.into_iter()
            .map(|result| {
...
mod tests {
    ...
    async fn test_comprehensive_search_workflow() {
        ...
        let scored_releases = search_service.search_and_rank(&search_request).await
        ...
        assert!(best_release.score > 100.0, "Best release should have high score: {}", best_release.score);
        
        // Verify request was made
        assert_eq!(search_service.get_request_count(), 1, "Should have made one request");
        
        println!("✅ Comprehensive search workflow test passed");
        println!("   Found {} releases, best score: {:.1}", scored_releases.len(), best_release.score);
    }
    ...
    async fn test_quality_scoring_algorithm() {
        ...
        // Score all releases
        let mut scored_releases: Vec<_> = releases.into_iter()
            .map(|release| {
        ...
        assert!(scored_releases[0].0.title.contains("2160p"), "4K UHD should score highest");
        assert!(scored_releases[0].0.indexer == "HDBits", "HDBits should score highest");
        assert!(scored_releases[0].1 > scored_releases[1].1, "UHD should score higher than 1080p");
        assert!(scored_releases[1].1 > scored_releases[2].1, "1080p should score higher than CAM");
        
        println!("✅ Quality scoring algorithm test passed");
        println!("   Scores: UHD={:.1}, 1080p={:.1}, CAM={:.1}", 
    ...
    async fn test_search_request_variations() {
        ...
        let test_cases = vec![
            ("IMDB ID search", SearchRequest {
                imdb_id: Some("tt0133093".to_string()),
                categories: vec![2000],
                ..Default::default()
            }),
            ("TMDB ID search", SearchRequest {
                tmdb_id: Some(603),
                categories: vec![2000],
                ..Default::default()
            }),
            ...
            ("Quality filtered search", SearchRequest {
                query: Some("The Matrix".to_string()),
                categories: vec![2000],
                min_seeders: Some(10),
                limit: Some(25),
        ...
        for (test_name, request) in test_cases {
            ...
            assert!(!releases.is_empty(), "Should find releases for {}", test_name);
        }
        
        // Verify request counting
        assert_eq!(search_service.get_request_count(), 4, "Should have made 4 requests");
    ...
    async fn test_concurrent_search_requests() {
        ...
        assert_eq!(successful_searches, 5, "All concurrent searches should succeed");
        assert_eq!(search_service.get_request_count(), 5, "Should have made 5 requests");
    ...
    async fn test_release_filtering_and_validation() {
        ...
        for scored_release in &scored_releases {
            ...
            assert!(!release.title.is_empty(), "Release title should not be empty");
            assert!(!release.download_url.is_empty(), "Download URL should not be empty");
            assert!(release.indexer_id > 0, "Indexer ID should be positive");
            assert!(!release.indexer.is_empty(), "Indexer name should not be empty");
            
            // Verify quality score is reasonable
            assert!(scored_release.score >= 0.0, "Score should be non-negative");
            assert!(scored_release.score <= 200.0, "Score should be reasonable (< 200)");
            ...
            assert!(release.categories.iter().any(|c| c.id == 2000), "Should be in Movies category");
