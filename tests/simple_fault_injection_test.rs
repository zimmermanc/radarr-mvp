...
async fn test_basic_circuit_breaker_functionality() {
    ...
    for i in 0..6 {
        ...
        match result {
            ...
            Err(RadarrError::CircuitBreakerOpen { service }) => {
                println!("Request {}: Circuit breaker opened (service: {})", i + 1, service);
                break;
            }
            other => {
    ...
    println!("✓ Final metrics: {} total, {} successful, {} failed, {} rejected", 
             metrics.total_requests, metrics.successful_requests, metrics.failed_requests, metrics.rejected_requests);
    
    assert!(failures >= 3, "Should have at least 3 failures");
    assert!(metrics.failed_requests >= 3, "Metrics should show at least 3 failures");
...
async fn test_timeout_scenario() {
    ...
    assert!(elapsed < Duration::from_secs(10), "Should timeout quickly: {:?}", elapsed);
...
async fn test_rate_limiting_scenario() {
    ...
    let result = context.circuit_breaker.call(async {
        let response = client.get(&limited_url).send().await
            .map_err(|e| RadarrError::NetworkError { 
                message: format!("HTTP request failed: {}", e) 
            })?;
    ...
    if let Err(RadarrError::RateLimited { service, retry_after }) = result {
        ...
        println!("✓ Rate limiting detected correctly: retry after {} seconds", retry_after.unwrap());
    } else {
    ...
    for result in results {
        ...
        match response {
            ...
            Err(RadarrError::CircuitBreakerOpen { .. }) => {
                ...
                println!("Request {}: Circuit breaker rejection in {:?}", request_id, elapsed);
            }
            other => {
                println!("Request {}: Unexpected result: {:?} in {:?}", request_id, other, elapsed);
            }
        }
    }
    
    assert!(service_errors > 0, "Should have service errors");
    println!("✓ Concurrent requests handled: {} service errors, {} circuit rejections", 
...
async fn test_complete_fault_injection_workflow() {
    ...
    for i in 0..4 {
        ...
        println!("Fault test {}: indexer={}, downloader={}", 
                 i + 1, indexer_result.is_ok(), downloader_result.is_ok());
    ...
    println!("Recovery: indexer={}, downloader={}", 
             indexer_recovery.is_ok(), downloader_recovery.is_ok());
    ...
    println!("\n--- Final Metrics ---");
    println!("Indexer: {} total, {} success, {} failed", 
             indexer_metrics.total_requests, indexer_metrics.successful_requests, indexer_metrics.failed_requests);
    println!("Downloader: {} total, {} success, {} failed",
             downloader_metrics.total_requests, downloader_metrics.successful_requests, downloader_metrics.failed_requests);
